...
% Variables x = [p0 p1 p2 p3 q01 q12 q13 g0 s2 s3]'.
% Objective: minimize w2*s2 + w3*s3
% Constraints:
%   Linear equalities (mass balance)
%   Nonlinear equalities (Weymouth on each pipe)
%   Bounds on pressures, flows, supply, shedding
...
%% ------------------------ Variable indexing ------------------------
% x = [ p0   p1   p2   p3   q01  q12  q13  g0   s2   s3 ]
...
%% ------------------------ Nonlinear equalities (Weymouth) ----------
% p0^2 - p1^2 = K01*q01^2
% p1^2 - p2^2 = K12*q12^2
% p1^2 - p3^2 = K13*q13^2
nonlcon = @(x) weymouth_ceq(x, i_p0, i_p1, i_p2, i_p3, i_q01, i_q12, i_q13, K01, K12, K13);
...
fprintf('Max Weymouth residual     : %.3e\n', norm(ceq, inf));
...
equality = Aeq * z - beq;
equality = [equality;
            z(i_p0)^2 - z(i_p1)^2 - K01*z(i_q01)^2;
            z(i_p1)^2 - z(i_p2)^2 - K12*z(i_q12)^2;
            z(i_p1)^2 - z(i_p3)^2 - K13*z(i_q13)^2];
...
%% ------------------------ Helper: Weymouth constraints --------------
function [c, ceq] = weymouth_ceq(x, i_p0,i_p1,i_p2,i_p3, i_q01,i_q12,i_q13, K01,K12,K13)
p0 = x(i_p0); p1 = x(i_p1); p2 = x(i_p2); p3 = x(i_p3);
q01 = x(i_q01); q12 = x(i_q12); q13 = x(i_q13);

% No inequality-type nonlinear constraints
c = [];

% Equalities (direction assumed nonnegative, so |q|=q)
ceq = zeros(3,1);
ceq(1) = (p0^2 - p1^2) - K01*q01^2;
ceq(2) = (p1^2 - p2^2) - K12*q12^2;
ceq(3) = (p1^2 - p3^2) - K13*q13^2;
end
