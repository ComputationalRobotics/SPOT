function tmp = PushBoxTunnel2_testplot(prefix, if_draw_image, if_draw_gif)
...
id = params.id; 
vertices_box = params.vertices_box;
% rescale variables
...
obs_circles = params.obs_circles;

if if_draw_image
    figure; hold on;
    % obstacles params
    obs_plot_params.lw = 0.5; obs_plot_params.color = [0, 0, 0]; obs_plot_params.alpha = 0.5;
    % final goal params
    final_plot_params.lw = 0.5; final_plot_params.color = [0.6, 0.2, 0.0]; final_plot_params.alpha = 0.5;
    % pusher params
    pusher_plot_params.s = 20; pusher_plot_params.alpha = 1.0;
    % slider params
    ...
    for ii = 1: num_step 
        ...
        px = p(1, k); py = p(2, k);
        Fx = F(1, k); Fy = F(2, k);
        [px, py] = tune_p(px, py, lam(:, k), 0.02, a, b);
        ...
        for i = 1: size(obs_circles, 1)
            obs = draw_circle(obs_circles(i, 1), obs_circles(i, 2), obs_circles(i, 3), obs_plot_params, t_params);
        end
        % draw final goal
...
if if_draw_gif
    ...
    ylim([y_min, y_max]);
    % obstacles params
    obs_plot_params.lw = 1.0; obs_plot_params.color = [0, 0, 0]; obs_plot_params.alpha = 1.0;
    % final goal params
    ...
    % draw obstacles
    for i = 1: size(obs_circles, 1)
        obs = draw_circle(obs_circles(i, 1), obs_circles(i, 2), obs_circles(i, 3), obs_plot_params);
    end
    % draw final goal
    ...
    for ii = 1: num_step 
        ...
        px = p(1, k); py = p(2, k);
        Fx = F(1, k); Fy = F(2, k);
        [px, py] = tune_p(px, py, lam(:, k), 0.015, a, b);
        scale_Fx = unit_length * Fx / F_max;
        scale_Fy = unit_length * Fy / F_max;
