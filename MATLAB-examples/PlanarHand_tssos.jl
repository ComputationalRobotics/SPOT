...
polys_file = matread("./data/PlanarHand_MATLAB/" * prefix_MATLAB * "polys.mat")
params_file = matread("./data/PlanarHand_MATLAB/" * prefix_MATLAB * "params.mat")
...
opt, sol, data = cs_tssos_first(F, v, kappa, numeq=length(eq_sys), CS=cs, cliques=[], TS=ts, solution=if_solution, QUIET=false)
println("Finish!")
...
decimal_cliques = [Int.(clique) for clique in data.cliques]
for (i, clique) in enumerate(decimal_cliques)
...
if if_solution
    ...
    # save data
    if !isdir("./data/PlanarHand_tssos/" * prefix_tssos)
        mkpath("./data/PlanarHand_tssos/" * prefix_tssos)
    end
    file = matopen("./data/PlanarHand_tssos/" * prefix_tssos * "data.mat", "w")  
...
if if_MATLAB && if_solution
    if MATLAB_filename == "v_opt_naive"
        println("Naive v_opt!")
        file = matread("./data/PlanarHand_MATLAB/" * prefix_MATLAB * "v_opt_naive.mat")
    ...
    elseif MATLAB_filename == "v_opt_robust"
        ...
        file = matread("./data/PlanarHand_MATLAB/" * prefix_MATLAB * "v_opt_robust.mat")
    ...
    elseif MATLAB_filename == "v_opt_ordered"
        ...
        file = matread("./data/PlanarHand_MATLAB/" * prefix_MATLAB * "v_opt_ordered.mat")
        sol_MATLAB = file["v_opt_ordered"]
        file_saved = "data_ordered_IPOPT.mat"
        ipopt_log_file = "ordered_IPOPT.txt"
    end
    
    if !isdir("./logs/PlanarHand_MATLAB/" * prefix_MATLAB)
        mkpath("./logs/PlanarHand_MATLAB/" * prefix_MATLAB)
    end
    logfile = open("./logs/PlanarHand_MATLAB/" * prefix_MATLAB * ipopt_log_file, "w")
    ...
    # save data again 
    file = matopen("./data/PlanarHand_MATLAB/" * prefix_MATLAB * file_saved, "w") 
