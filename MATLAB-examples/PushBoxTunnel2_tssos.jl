...
polys_file = matread("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * "polys.mat")
params_file = matread("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * "params.mat")
...
opt, sol, data = cs_tssos_first(F, v, kappa, numeq=length(eq_sys), CS=cs, cliques=cliques, TS=ts, solution=false, QUIET=false)
println("Finish!")

open("julia_solution.txt", "a") do f
    println(f, "PushBoxTunnel2    TS=$ts, CS=$cs, result=$(string(round(opt, digits=20))), operation = , mosek = ")
end
...
if if_solution
    ...
    # save data
    if !isdir("./data/PushBoxTunnel2_tssos/" * prefix_tssos)
        mkpath("./data/PushBoxTunnel2_tssos/" * prefix_tssos)
    end
    file = matopen("./data/PushBoxTunnel2_tssos/" * prefix_tssos * "data.mat", "w")  
...
if if_MATLAB && if_solution
    if MATLAB_filename == "v_opt_naive"
        println("Naive v_opt!")
        file = matread("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * "v_opt_naive.mat")
    ...
    elseif MATLAB_filename == "v_opt_robust"
        ...
        file = matread("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * "v_opt_robust.mat")
    ...
    elseif MATLAB_filename == "v_opt_ordered"
        ...
        file = matread("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * "v_opt_ordered.mat")
        sol_MATLAB = file["v_opt_ordered"]
        file_saved = "data_ordered_IPOPT.mat"
        ipopt_log_file = "ordered_IPOPT.txt"
    end
    
    if !isdir("./logs/PushBoxTunnel2_MATLAB/" * prefix_MATLAB)
        mkpath("./logs/PushBoxTunnel2_MATLAB/" * prefix_MATLAB)
    end
    logfile = open("./logs/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * ipopt_log_file, "w")
    ...
    # save data again 
    file = matopen("./data/PushBoxTunnel2_MATLAB/" * prefix_MATLAB * file_saved, "w") 
